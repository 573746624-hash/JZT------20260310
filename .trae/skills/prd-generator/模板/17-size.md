...
| :--- | :--- | :--- | :--- | :--- |
| v1.0.0 | 2026-03-24 | 初始版本，完成Size管理基础功能PRD | 张博 | - |
| v1.0.1 | 2026-03-25 | 优化导入导出功能，增加数据校验规则 | 张博 | 李明 |
| v1.1.0 | 2026-04-01 | 新增Where Used查询功能，完善业务规则 | 张博 | 王芳 |

---

## 1. 页面内说明

本页面主要用于维护和管理产品尺寸（Size）的基础数据，支持一维（1D）和二维（2D）尺寸的创建与配置。页面提供尺寸的增删改查、列表排序调整、批量导入导出以及关联引用（Where Used）查询功能。核心目标是建立标准化、唯一性的尺寸字典，供下游商品和库存系统统一调用，避免尺寸数据的混乱与冗余。

## 2. 页面元素描述

### 2.1 搜索与筛选区

| 元素名称 | 元素类型 | 默认值 | 是否必填 | 校验规则 | 状态变化说明 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| Tab 切换器 | 标签页 | Style | 是 | 仅支持 Style/Material 切换 | 切换时清空筛选条件，重新加载对应类型数据 |
| 2D? 筛选框 | 下拉单选 | All 2D? | 否 | 无 | 选中后实时触发列表刷新 |
| 搜索框 | 文本输入 | 空 | 否 | 最长50字符，过滤特殊符号 | 监听输入，500ms 防抖后触发模糊搜索 |

### 2.2 列表展示区

| 元素名称 | 元素类型 | 默认值 | 是否必填 | 校验规则 | 状态变化说明 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 数据表格 | 数据组件 | 空 | 否 | 无 | 数据为空时展示占位图；加载中展示骨架屏 |
| 分页器 | 翻页组件 | 1页/10条 | 是 | 依据总条数计算总页数 | 切换页码或每页条数时，触发列表数据重载 |
| 引用状态(Used) | 状态标签 | - | - | 枚举值：Yes/No | 值为 Yes 时，该行操作列自动隐藏 Delete 按钮 |

### 2.3 弹窗表单区 (新增/编辑)

| 元素名称 | 元素类型 | 默认值 | 是否必填 | 校验规则 | 状态变化说明 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| Size | 文本输入 | 空 | 否 | 系统生成，不可编辑 | 根据维度选择自动拼接生成，始终置灰只读 |
| 2D? | 下拉单选 | No | 是 | 新增可选，编辑时只读 | 选中 Yes 后，动态渲染 Dimension2 及 Sort by2 字段 |
| Sort by1 | 数字输入 | 空 | 是 | 1-9999 正整数，同维度唯一 | 2D 模式下继承所选 1D 的排序号并锁定只读 |
| Dimension1 | 复合输入 | 空 | 是 | 最长20字符 | 1D 模式下为文本输入；2D 模式下为下拉选择（加载已有 1D） |
| Dimension2 | 下拉单选 | 空 | 2D必填 | 必须存在于 Size Config | 仅在 2D 模式下显示，选中后触发 Size 字段重新生成 |
| Sort by2 | 数字输入 | 空 | 2D必填 | 1-9999 正整数，局部唯一 | 仅在 2D 模式下显示，控制二维内的显示顺序 |
| Description | 多行文本 | 空 | 否 | 最长200字符 | 支持回车换行 |
