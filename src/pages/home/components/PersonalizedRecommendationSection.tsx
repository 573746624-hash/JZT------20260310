...
import { Card, Row, Col, Typography, Tag, Button, Avatar, List, Divider, Badge, Tooltip, Descriptions, Space, Empty, Spin } from 'antd';
...
export const PersonalizedRecommendationSection: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const matchedList = allRecommendations.map(item => {
        let score = 50; // 基础分
        ...
        // 匹配逻辑 4: 营收相关 (金融服务匹配)
        if (profile.revenue && parseInt(profile.revenue.replace(/[^0-9]/g, '')) > 100) {
        ...
        const finalScore = Math.min(99, score + Math.floor(Math.random() * 5));
    ...
    <Row gutter={[16, 16]}>
      ...
      <Col xs={24} lg={8}>
        <Card
          ...
          bodyStyle={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', height: 'calc(100% - 58px)' }}
        >
          <div style={{ flex: 1, overflowY: 'auto', paddingRight: '8px' }}>
            ...
            <div style={{ marginBottom: '16px' }}>
              <Descriptions column={1} size="small" labelStyle={{ color: '#8c8c8c' }} contentStyle={{ fontWeight: 500 }}>
                ...
                <Descriptions.Item label="注册地址">{profile.address || '--'}</Descriptions.Item>
              </Descriptions>
            </div>
                ...
                <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Empty 
                    ...
                    description={
                      <span>
                        暂无推荐内容<br/>
                        <Text type="secondary" style={{ fontSize: 13 }}>请先在左侧完善企业名称、行业或规模等画像信息，解锁为您量身定制的政策与服务。</Text>
                      </span>
                    }
                  />
                </div>
                      ...
                      <div style={{ width: '100%' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                          <div style={{ flex: 1 }}>
                            ...
                            <Paragraph 
                              style={{ margin: 0, fontSize: '13px', color: '#666', marginBottom: '8px' }}
                              ellipsis={{ rows: 2 }}
                            >
                              {item.description}
                            </Paragraph>
                            ...
                            {item.matchReasons && item.matchReasons.length > 0 && (
                              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '8px' }}>
                                {item.matchReasons.map((reason, idx) => (
                                  <Text key={idx} type="success" style={{ fontSize: '12px', background: '#f6ffed', padding: '2px 8px', borderRadius: '4px', border: '1px solid #b7eb8f' }}>
                            ...
                            {item.updateTag && (
                              <div style={{ marginBottom: '8px' }}>
                                <span style={{ 
                                  ...
                                  alignItems: 'center',
                                  padding: '4px 8px', 
                                  background: item.updateTag.type === 'hot' ? '#fff1f0' : item.updateTag.type === 'urgent' ? '#fff7e6' : '#e6f7ff',
                                  color: item.updateTag.type === 'hot' ? '#f5222d' : item.updateTag.type === 'urgent' ? '#fa8c16' : '#1890ff',
                                  borderRadius: '4px',
                                  fontSize: '12px',
                                  fontWeight: 500,
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            {/* 标签区分服务类型 */}
                            <Tag color={item.type === 'policy' ? 'blue' : item.type === 'finance' ? 'gold' : 'purple'}>
                              {item.type === 'policy' ? '政策补贴' : item.type === 'finance' ? '金融服务' : '企业服务'}
                            </Tag>
                            ...
                            {item.type === 'policy' ? (
                              ...
                              <>
                                ...
                                {item.score && <Tag color="orange" icon={<StarOutlined />}>{item.score}</Tag>}
                              </>
                            )}
                          </div>
                          <div style={{ display: 'flex', alignItems: 'center' }}>
